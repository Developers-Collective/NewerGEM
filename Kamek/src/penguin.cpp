...
////////////////////////
	...
	bool daPengi::collisionCatD_Drill(ActivePhysics *apThis, ActivePhysics *apOther) {
		this->doStateChange(&StateID_Die);
        return false;
	}

	bool daPengi::collisionCat7_GroundPound(ActivePhysics *apThis, ActivePhysics *apOther) {
		this->doStateChange(&StateID_Die);
        return false;
	}

	bool daPengi::collisionCat7_GroundPoundYoshi(ActivePhysics *apThis, ActivePhysics *apOther) {
		this->doStateChange(&StateID_Die);
        return false;
	}

	bool daPengi::collisionCat9_RollingObject(ActivePhysics *apThis, ActivePhysics *apOther) {
		this->doStateChange(&StateID_Die);
        return false;
	}

	bool daPengi::collisionCat3_StarPower(ActivePhysics *apThis, ActivePhysics *apOther){
		this->doStateChange(&StateID_Die);
        return false;
	}

	bool daPengi::collisionCatA_PenguinMario(ActivePhysics *apThis, ActivePhysics *apOther){
		this->doStateChange(&StateID_Die);
        return false;
	}

	bool daPengi::collisionCat14_YoshiFire(ActivePhysics *apThis, ActivePhysics *apOther){
		this->doStateChange(&StateID_DieSmoke);
        return false;
	}

	bool daPengi::collisionCat1_Fireball_E_Explosion(ActivePhysics *apThis, ActivePhysics *apOther) {
		this->doStateChange(&StateID_DieSmoke);
        return false;
	}
	...
	bool daPengi::collisionCat13_Hammer(ActivePhysics *apThis, ActivePhysics *apOther) {
		this->doStateChange(&StateID_Die);
        return false;
	}
...
bool daPengi::calculateTileCollisions() {
	...
	cmgr_returnValue = this->collMgr.isOnTopOfTile();
	...
	float xDelta = this->pos.x - this->last_pos.x;
	if (xDelta >= 0.0f) {
		this->direction = 0;
	} else {
		this->direction = 1;
	...
	if (this->collMgr.isOnTopOfTile()) {
		...
		this->max_speed.x = (direction == 1) ? -1.0f : 1.0f;
	} else {
	...
	// Switch Direction
	if (collMgr.outputMaybe & (0x15 << this->direction)) {
