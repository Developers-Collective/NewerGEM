...
void cppProcessHeap(u32 freeSize, u32 heapSize, int heapID, char *name) {
	...
	for (int i = 0; i < 4; i++) {
		GXPosition3f32(16*i+8, END_AT, 0.0);
		GXColor1u32(col_5758[i]);
		GXPosition3f32(16*i+16+8, END_AT, 0.0);
		GXColor1u32(col_5758[i]);
		GXPosition3f32(16*i+16+8, END_AT+16.0f, 0.0);
		GXColor1u32(col_5758[i]);
		GXPosition3f32(16*i+8, END_AT+16.0f, 0.0);
		GXColor1u32(col_5758[i]);
	}
	...
	for (int i = 0; i < 4; i++) {
		if (Global5758 & (1 << i)) {
			...
			GXPosition3f32(16*i+16+8, END_AT+16.0f, 0.0);
			GXColor1u32(0xffffffff);
			GXPosition3f32(16*i+16+8, END_AT+24.0f, 0.0);
			GXColor1u32(0xffffffff);
			GXPosition3f32(16*i+8, END_AT+24.0f, 0.0);
			GXColor1u32(0xffffffff);
	...
	for (int i = 0; i < 8; i++) {
		if (GlobalEnableFlag & (1 << i)) {
			...
			GXPosition3f32(16*i+8, END_AT+32.0f, 0.0);
			GXColor1u32(0xffffffff);
			GXPosition3f32(16*i+16+8, END_AT+32.0f, 0.0);
			GXColor1u32(0xffffffff);
			GXPosition3f32(16*i+16+8, END_AT+40.0f, 0.0);
			GXColor1u32(0xffffffff);
			GXPosition3f32(16*i+8, END_AT+40.0f, 0.0);
			GXColor1u32(0xffffffff);
