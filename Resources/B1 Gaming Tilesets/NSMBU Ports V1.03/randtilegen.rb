...
nohara_clones = %w(nohara nohara_staffroll_u cracks springwater space chika2)
...
g.section('Pa1_chika_u', 'Pa1_chika2_u', 'Pa1_daishizen_u', 'Pa1_daishizen_obake_u', 'Pa1_daishizen_toso_u', 'Pa1_gake_u', 'Pa1_nohara2_u', 'Pa1_nohara3_u', 'Pa1_nohara4_u', 'Pa1_obake_soto_u', 'Pa1_suichu_u', 'Pa1_suichul_u') do
	g.regular_u_terrain
end

g.section('Pa1_nohara_u', 'Pa1_chikal_u', 'Pa1_kaigan_u', 'Pa1_kazan_u', 'Pa1_setsugen_u', 'Pa1_setsugenl_u') do
    g.random(0x12..0x15, :horz)
    g.random(0x22..0x25, :horz)
    g.random([0x10, 0x20, 0x30, 0x40], :vert)
    g.random([0x11, 0x21, 0x31, 0x41], :vert)
end

g.section('Pa1_koopa_out_u') do
    g.random(0x12..0x15, :horz)
    g.random(0x22..0x25, :horz)
    g.random([0x10, 0x20, 0x30, 0x40], :vert)
    g.random([0x11, 0x21, 0x31, 0x41], :vert)
...
g.section('Pa1_noharal_u') do
    g.random([0x12, 0x13, 0x14, 0x15, 0xB3, 0xB4])
    g.random([0x10, 0x20, 0x30, 0x40], :vert)
    g.random([0x11, 0x21, 0x31, 0x41], :vert)
end

g.section('Pa2_nohara_u') do
    g.random([0xCF, 0xDF, 0xEF, 0xFF])
end
g.section('Pa2_doukutu_u', 'Pa2_doukutu2_u', 'Pa2_doukutu3_u', 'Pa2_doukutu4_u', 'Pa2_doukutu5_u', 'Pa2_doukutu6_u', 'Pa2_doukutu7_u', 'Pa2_doukutu8_u') do
    g.random([0x19, 0x29, 0x1A, 0x2A], :horz)
    g.random([0x6E, 0x6F, 0x7E, 0x7F])
    g.random([0x59, 0x5A, 0x4D, 0x4E], :horz)
    g.random([0x68, 0x69, 0x78, 0x79], :vert)
    g.random([0x8E, 0x8F, 0x9E, 0x9F], :vert)
end
